/**
 * Migrate cột shoot: INTEGER -> REAL cho tất cả bảng máy hiện có.
 *
...
async function getShootColumnType(machineId) {
  const { rows } = await pool.query(
    `SELECT data_type
     FROM information_schema.columns
     WHERE table_schema = 'public'
       AND table_name = $1
       AND column_name = 'shoot'`,
    [machineId],
  );
...
async function migrateMachineTable(machineId) {
  ...
  const dataType = await getShootColumnType(machineId);
  if (!dataType) {
    console.log(`[skip] ${machineId}: không có cột shoot`);
    return 'missing_column';
  }

  if (REAL_TYPES.has(dataType)) {
    console.log(`[ok]   ${machineId}: shoot đã là ${dataType}`);
    return 'already_real';
  }

  await pool.query(
    `ALTER TABLE "${machineId}" ALTER COLUMN shoot TYPE REAL USING shoot::real`,
  );
  console.log(`[done] ${machineId}: shoot ${dataType} -> REAL`);
  return 'migrated';
}
